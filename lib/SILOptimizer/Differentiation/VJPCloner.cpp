...
VJPCloner::Implementation::buildPullbackValueTupleValue(TermInst *termInst) {
  ...
  auto origBB = termInst->getParent();
  llvm::errs() << "\nBEFORE getLinearMapTupleLoweredType\n";
  auto tupleLoweredTy =
      remapType(pullbackInfo.getLinearMapTupleLoweredType(origBB));
  llvm::errs() << "AFTER getLinearMapTupleLoweredType\n";
  auto bbPullbackValues = getPullbackValues(origBB);
  TupleInst *tmp = getBuilder().createTuple(loc, tupleLoweredTy, bbPullbackValues);

  llvm::errs() << "TUPLE 02: " << *tmp;
  llvm::errs() << "tupleLoweredTy: " << tupleLoweredTy << "\n\n";
