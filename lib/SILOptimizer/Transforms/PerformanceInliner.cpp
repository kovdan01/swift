...
static PartialApplyInst *getTopLevelPullback(SILFunction *vjp) {
  ...
  auto handleConvertFunctionOrPartialApply = [](SILInstruction *inst) -> PartialApplyInst * {
  ...
  if (auto *ti = dyn_cast<TupleInst>(retValDefInst)) {
    SILInstruction *lastElemDefInst = ti->getOperand(ti->getNumOperands() - 1).getDefiningInstruction();
...
static bool vjpHasTopLevelSpecializableClosures(SILFunction *vjp) {
  ...
  for (SILValue arg : topLevelPullback->getArguments()) {
    if (arg->getType().getASTType()->getAs<SILFunctionType>() != nullptr) {
      // Be conservative, treat any function type as potential specializable closure
      return true;
    }
  }
...
static bool isProfitableToInlineAutodiffVJP(FullApplySite applySiteOfVJP,
  ...
  bool calleeHasTopLevelSpecClosures = vjpHasTopLevelSpecializableClosures(calleeVJP);
  ...
  //   if (hasPullbackOnlyDirectUses(applySiteOfVJP)) // TODO: what if we have branch tracing enum?
  //     return true;
