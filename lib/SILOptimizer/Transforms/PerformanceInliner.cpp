...
static bool isTrivialVJP(SILFunction *vjp) {
  ...
  for (auto &bb : *vjp) {
    for (auto &inst : bb) {
      if (auto *ai = dyn_cast<FullApplySite>(&inst)) {
...
static bool isProfitableToInlineAutodiffVJP(FullApplySite applySiteOfVJP,
  ...
  // Always consider derivative thunks for inlining. It does not harm ADCS since explicit derivative is by itself subject for ADCS optimization.
  if (calleeVJP->isThunk() == IsThunk_t::IsThunk) {
    return true;
  }

  // Inlining of VJPs which are guaranteed to gain no optimization effect from ADCS does not harm ADCS effectiveness (we know in advance that it would not be effective). Always consider for inlining.
  if (isTrivialVJP(calleeVJP)) {
    return true;
  }

  // After inlining to a non-VJP caller, we can no longer run ADCS for the callee VJP. Skip inlining for now.
  if (!isCallerVJP) {
    return false;
  }
  ...
  if (isCallerVJP && callerHasControlFlow) {
    // If pullback extracted from this VJP call result is only passed to top-level pullback of caller VJP, consider this callee VJP for inlining. It does not harm ADCS effectiveness since it properly handles nested inlining by running multiple specialization rounds and immediately folding partial_apply+apply pairs in the specialized caller pullback.
