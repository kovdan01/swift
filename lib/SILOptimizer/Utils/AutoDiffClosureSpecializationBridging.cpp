...
static llvm::SmallVector<Type, 8> getBranchTracingEnumPreds(Type bteType) {
  ...
  for (EnumElementDecl *oldEED : ed->getAllElements()) {
...
static BridgedType autodiffSpecializeBranchTracingEnum(
  ...
  for (EnumElementDecl *oldEED : oldED->getAllElements()) {
    ...
    for (unsigned idxInPayloadTuple = 0;
      ...
      if (closureAndIdxInPayloadIt != closureAndIdxList.end()) {
        newEEDNameSuffix += '_' + std::to_string(idxInPayloadTuple);
    ...
    if (!newEEDNameSuffix.empty())
      newEDNameStr += '_' + oldEED->getNameStr().str() + newEEDNameSuffix;
  }
  ...
  for (auto [idx, oldEED] : llvm::enumerate(oldED->getAllElements())) {
