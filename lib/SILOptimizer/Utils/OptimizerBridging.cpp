...
bool BridgedType::isAutodiffBranchTracingEnumInVJP(BridgedFunction vjp) const {
  ...
  if (!edName.starts_with("_AD__"))
    return false;
  if (!llvm::StringRef(edName.data() + 5, edName.size() - 5).starts_with(MANGLING_PREFIX_STR))
    return false;
  ...
  // Thus, "_AD__", MANGLING_PREFIX and "_bb" must have total length less than idx.
  ...
  llvm::StringRef enumOrigFuncName =
      std::string_view(edName.data() + 5 + manglingPrefixSize, idx - (5 + manglingPrefixSize + 3));

  Demangle::Context Ctx;
  if (auto *root = Ctx.demangleSymbolAsNode(vjp.getFunction()->getName()))
    if (auto *node = root->findByKind(Demangle::Node::Kind::Function, /*maxDepth=*/3))
