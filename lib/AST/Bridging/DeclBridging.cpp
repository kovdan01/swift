...
// Define `.asValueDecl` on each BridgedXXXDecl type.
#define DECL(Id, Parent)
...
// Define `.asNominalTypeDecl` on each BridgedXXXDecl type.
#define DECL(Id, Parent)
...
BridgedEnumDecl BridgedEnumDecl_create(
  ...
  auto *decl = new (context)
      EnumDecl(SourceLoc{}, context.getIdentifier(name.unbridged()), SourceLoc{},
               {},
               genericParamList.unbridged(), cDeclContext.unbridged());
...
BridgedEnumElementDecl BridgedEnumElementDecl_create(
  ...
  return new (context) EnumElementDecl(
      ...
      DeclName(context.getIdentifier(name.unbridged())), parameterList.unbridged(),
      SourceLoc(), /*RawValueExpr*/ nullptr, cDeclContext.unbridged());
}
...
BridgedParameterList BridgedParameterList_create(
  ...
  return ParameterList::create(cContext.unbridged(), cParameters.unbridged<ParamDecl *>());
}
