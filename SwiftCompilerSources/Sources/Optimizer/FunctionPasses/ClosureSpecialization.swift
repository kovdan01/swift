    ...
    for bb in bbQueue {
      ...
      if let successor = throwingSuccessor {
        ...
        let newArg = specializeOptionalBBArgInPullback(
          bb: successor, newOptionalType: enumCase.enumType.loweredTypeWithAbstractionPattern(in: bb.parentFunction).getEnumCases(in: bb.parentFunction)![enumCase.index]!.payload!.tupleElements.last!, context: cloner.context)
...
func autodiffSpecializeBranchTracingEnums(
  ...
  for bteType in bteQueue {
    ...
    for closureInfoMultiBB in closureInfosMultiBB {
      if remapType(ty: closureInfoMultiBB.enumCase.enumType.loweredTypeWithAbstractionPattern(in: topVJP), function: topVJP) != bteType {
        continue
      }
  ...
  -> PullbackClosureInfo
{
  ...
  for closureInfo in pullbackClosureInfo.closureInfosMultiBB {
    ...
    let bteCaseName = enumCase.enumType.loweredTypeWithAbstractionPattern(in: vjp).getEnumCases(in: vjp)![enumCase.index]!.name
    // In terms of the example above, bteCaseName is now equal to "bbK".

    let bteWithNoneArr = branchTracingEnumInstArr.filter {
      $0.caseName == bteCaseName && $0.type.rawType != enumCase.enumType
    }
...
let getPullbackClosureInfoMultiBBTest = FunctionTest(
  ...
  for closureInfoMultiBB in pullbackClosureInfo.closureInfosMultiBB {
    ...
    print("      enumCase: \(enumCase.enumType.loweredTypeWithAbstractionPattern(in: function)).\(enumCase.name)")
...
let specializePayloadArgInPullbackBB = FunctionTest("autodiff_specialize_payload_arg_in_pb_bb") {
  ...
  for bb in pb.blocks {
    ...
    let enumType = enumDict[enumCase.enumType.loweredTypeWithAbstractionPattern(in: function)]!
    let newArg = specializePayloadTupleBBArgInPullback(
      arg: arg,
