...
let autodiffClosureSpecialization = FunctionPass(name: "autodiff-closure-specialization") {
  ...
  if !isSingleBB {
    ...
    if pbApply!.referencedFunction == nil {
      return
    }
    debugPrint("AAAAA PB BEFORE BEGIN")
    debugPrint(pbApply!.referencedFunction!)
...
private func getOrCreateSpecializedFunctionCFG(
  basedOn callSite: CallSite, enumDict: inout EnumDict, bbMap: [BasicBlock:BasicBlock], _ context: FunctionPassContext
)
  -> (function: Function, alreadyExists: Bool)
{
  ...
  context.buildSpecializedFunction(
    ...
    buildFn: { (emptySpecializedFunction, functionPassContext) in
      ...
      closureSpecCloner.cloneAndSpecializeFunctionBodyCFG(
        using: callSite, /*enumType: enumType, */enumDict: &enumDict, bbMap: bbMap)
    })
...
private func updateCallSiteCFG(
  ...
  for closureInfo in closureInfoArr {
    ...
    if succBB.arguments[0].bridged.hasOneUse() {
      let possibleDestructureTupleInst = BridgedOperand(
...
private func getEnumPayloadArgOfPbBB(_ bb: BasicBlock) -> Optional<Argument> {
  var argOpt = Optional<Argument>(nil)
  for arg in bb.arguments {
    if !arg.type.isTuple {
      continue
    ...
    case let ti as TupleInst:
        ...
        ti == returnInst.returnedValue
      {
        ...
        for tiUse in ti.uses {
          ...
          let enumTypeAndCase = EnumTypeAndCase(enumType: ei.type, caseIdx: ei.caseIndex)
          closureInfoArr.append(ClosureInfoCFG(
...
private func rewriteUsesOfPayloadItem(
  ...
  debugPrint("AAAAAA rewriteUsesOfPayloadItem BEGIN")
  debugPrint(resultIdx)
...
//        let vjpBB = closureInfo.closureInfo.closure.parentBlock
//        if getBBIdxInFunc(bbMap[vjpBB]!) == newDtiBlockIdx {
          assert(closureInfoOpt == nil)
...
//        }
  ...
  debugPrint("AAAAAA rewriteUsesOfPayloadItem END")
}

private func getBBIdxInFunc(_ bb: BasicBlock) -> Int {
  for (idx, block) in bb.parentFunction.blocks.enumerated() {
    if block == bb {
      return idx
    }
  }
...
extension SpecializationCloner {
  fileprivate func cloneAndSpecializeFunctionBodyCFG(
    ...
    let entryEnumType = getEnumArgOfEntryPbBB(self.cloned.entryBlock)
    let closureInfos = callSite.closureInfosWithApplyCFG
    ...
    bbWorklist.pushIfNotVisited(self.cloned.entryBlock)
...
//        assert(needRewriteEnum(enumType: arg.type, closureInfosWithApplyCFG: closureInfos))
    ...
    for bb in self.cloned.blocks {
      ...
      var vjpBBOpt = Optional<BasicBlock>(nil)
      ...
      for (opIdx, op) in lastTI.operands.enumerated() {
        ...
        for closureInfo in closureInfos {
          if closureInfo.closureInfo.closure == val {
            if bbMap[vjpBB] == closureInfo.applyInPb.parentBlock {
              assert(closureInfo.closureInfo.idxesInEnumPayload[0] == opIdx)
              closureInfoArray.append(closureInfo)
              rewriter.appendToClosuresBufferForPb(
                closureInfo.closureInfo.closure.bridged,
                closureInfo.closureInfo.idxesInEnumPayload[0])
            }
          }
        }
      }
...
//    self.entryBlock.bridged.eraseInstruction(entrySwitchEnum.bridged)
  ...
  fileprivate func cloneAndSpecializeFunctionBodyCFGOld(
    using callSite: CallSite, /*enumType: Type, */enumDict: inout EnumDict, bbMap: [BasicBlock:BasicBlock]
  ) {
    self.cloneEntryBlockArgsWithoutOrigClosuresCFG(
    ...
    var closureInfoByEnumCase = [Int: [ClosureInfoWithApplyCFG]]()
    for closureInfo in closureInfos {
      var tmp: [ClosureInfoWithApplyCFG]? = closureInfoByEnumCase[
        closureInfo.closureInfo.enumTypeAndCase.caseIdx]
      if tmp == nil {
...
extension Builder {
  ...
  fileprivate func cloneRootClosureReabstractions(
    ...
    -> (
      finalClonedReabstractedClosure: SingleValueInstruction,
      releasableClonedReabstractedClosures: [PartialApplyInst]
    )
  {
    func inner(
      ...
      case let mdi as MarkDependenceInst:
        let toBeReabstracted = inner(
          rootClosure, clonedRootClosure, mdi.value, &releasableClonedReabstractedClosures,
    ...
    let finalClonedReabstractedClosure = inner(
      ...
      &releasableClonedReabstractedClosures, &origToClonedValueMap)
    return (
      finalClonedReabstractedClosure as! SingleValueInstruction,
      releasableClonedReabstractedClosures
    )
  }
...
private struct CallSite {
  ...
  func specializedCalleeNameCFG(_ context: FunctionPassContext) -> String {
    ...
    return context.mangle(
      withEnumArguments: [closureInfosWithApplyCFG[0].closureInfo.closure], enumArgIndices: [0],
      from: applyCallee)
  }
}
