...
func getPayloadValues(payload: Argument, vjp: Function) -> PayloadValues {
  ...
  for use in payload.uses {
    ...
    if tei.fieldIndex == 0 && tei.type.isBranchTracingEnum(vjp: vjp)
  ...
  -> PullbackClosureInfo?
{
  ...
  if pullbackClosureInfoOpt != nil {
    ...
    for closureInfo in pullbackClosureInfoOpt!.closureInfosCFG {
      ...
      let branchTracingEnumCaseName = closureInfo.enumTypeAndCase.enumType.getEnumTypeCaseName(
        caseIdx: closureInfo.enumTypeAndCase.caseIdx)
      for inst in caller.instructions {
...
private func rewriteApplyInstructionCFG(
  ...
  for inst in vjp.instructions {
    ...
    guard let newEnumTypeBridged = enumDict[ei.results[0].type.bridged] else {
      continue
    }
    let newEnumType = newEnumTypeBridged.type
    ...
    for closureInfo in closureInfos {
      ...
      assert(
        (closureInfo.subsetThunk == nil && ti.operands[idxInTuple].value == closureInfo.closure)
          ...
          || (closureInfo.throwingInfo != nil
            && (closureInfo.throwingInfo!.isNone
    ...
    for (opIdx, op) in ti.operands.enumerated() {
      ...
      if tupleIdxToCapturedArgs[opIdx]!.isOptionalSome != nil {
        if tupleIdxToCapturedArgs[opIdx]!.isOptionalSome! {
          ...
          let optOfTuple =
            builderPred.bridged.createOptionalSome(tuple.bridged).instruction as! EnumInst
          newPayloadValues.append(optOfTuple)
        } else {
          let optOfTuple = builderPred.createOptionalNone(
            tupleElements: tupleIdxToCapturedArgs[opIdx]!.values)
          newPayloadValues.append(optOfTuple)
        }
      } else {
        let tuple = builderPred.createTuple(elements: tupleIdxToCapturedArgs[opIdx]!.values)
    ...
    if tiOpt == nil {
          ...
          with: "// autodiff subset parameters thunk for")
      {
        ...
        if maybeSubsetThunkOpt != nil {
          ...
          if maybeOptionalOpt != nil && maybeOptionalOpt!.operands.count == 1
            && maybeOptionalOpt!.type.isOptional
          {
  ...
  case let sei as SwitchEnumInst:
    ...
    if newSEI.operands[0].value.type.isOptional {
      ...
      let newArg = specializeOptionalBBArgInPullback(
        succ.bridged, newSEI.operands[0].value.type.bridged
      )
      .argument
      for argUse in newArg.uses {
        rewriteUsesOfPayloadItem(
          use: argUse, resultIdx: resultIdx, closureInfoArray: closureInfoArray,
          result: newArg /*result*/, useTei: useTei, context: context)
      }
    }
        ...
        for (opIdx, op) in tiInVjp!.operands.enumerated() {
          ...
          for closureInfo in closureInfos {
            if ((closureInfo.subsetThunk == nil && closureInfo.closure == val)
              ...
              || (closureInfo.throwingInfo != nil
                && (closureInfo.closure.uses.singleUse!.instruction as! EnumInst) == val))
...
private func getSpecializedParametersCFG(
  ...
  for paramInfo in applySiteCallee.convention.parameters {
    ...
    let newParamInfo = ParameterInfo(
      type: enumDictCopy[enumType.bridged]!.mapTypeOutOfContext().type.canonicalType,
