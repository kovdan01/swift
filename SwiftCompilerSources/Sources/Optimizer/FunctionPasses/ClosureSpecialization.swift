...
private func getSpecializedParametersCFG(
  ...
  for paramInfo in applySiteCallee.convention.parameters {
    if paramInfo.type != enumType.mapTypeOutOfContext(in: pb).canonicalType {
    ...
    let newParamInfo = ParameterInfo(
      type: enumDict[enumType]!.mapTypeOutOfContext(in: pb).canonicalType,
...
//      continue
//    }
//    guard let closure = svi.asSupportedClosure else {
//      continue
//    }
...
private func rewriteUsesOfPayloadItem(
  ...
  case let uedi as UncheckedEnumDataInst:
    ...
    let newUedi = builder.createUncheckedEnumData(
      ...
      resultType: result.type.getEnumCase(in: uedi.parentFunction, at: uedi.caseIndex)!.payload!)
