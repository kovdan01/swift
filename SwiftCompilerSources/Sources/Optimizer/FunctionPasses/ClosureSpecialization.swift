    ...
    for bb in bbQueue {
      ...
      if let successor = throwingSuccessor {
        ...
        let newArg = specializeOptionalBBArgInPullback(
          bb: successor, newOptionalType: enumCase.enumType(in: bb.parentFunction).getEnumCases(in: bb.parentFunction)![enumCase.index]!.payload!.tupleElements.last!, context: cloner.context)
