  ...
  -> (function: Function, alreadyExists: Bool)
{
  ...
  let specializedFunction = 
    context.createFunctionForClosureSpecialization(from: pullbackFn, withName: specializedFunctionName, 
                                                   withParams: specializedParameters, 
                                                   withSerialization: pullbackFn.isSerialized)
...
private extension SpecializationCloner {
  fileprivate func cloneAndSpecializeFunctionBodyCFG(
    ...
    while bbVisited.count != self.cloned.blocks.count {
