...
extension BridgedDecl {
  public var asDeclObj: BridgedDeclObj { BridgedDeclObj(SwiftObject(raw.bindMemory(to: BridgedSwiftObject.self, capacity: 1))) }
...
private func checkIfCanRun(vjp: Function, context: FunctionPassContext) -> Bool {
  ...
  for pbBB in pb.blocks {
    guard let (argOfPbBB, _) = getBTEPayloadArgOfPbBBWithBTETypeAndCase(pbBB, vjp: vjp) else {
      continue
    }
...
func remapType(ty: SIL.`Type`, function: Function) -> SIL.`Type` {
  ...
  if silType.rawType.hasArchetype {
    silType = silType.mapTypeOutOfContext()
  }
  let remappedCanType = silType.rawType.getReducedType(sig: function.loweredFunctionType.SILFunctionType_getSubstGenericSignature().genericSignature)
  let remappedSILType = Type.getPrimitiveType(canType: remappedCanType, silValueCategory: silType.category)
...
func getSourceFileFor(derivative: Function) -> BridgedSourceFile {
  if let sourceFileRawPtr = derivative.sourceFile.raw {
    return BridgedSourceFile(raw: sourceFileRawPtr)
  }
  return derivative.bridged.getFilesForModule().withElements(ofType: BridgedFileUnit.self) {
    for fileUnit in $0 {
      if let sourceFileRawPtr = fileUnit.castToSourceFile().raw {
        return BridgedSourceFile?(BridgedSourceFile(raw: sourceFileRawPtr))
      }
    }
    assert(false)
    return nil
...
func cloneGenericParameters(astContext: BridgedASTContext, declContext: BridgedDeclContext, canGenericSig: CanGenericSignature) -> BridgedGenericParamList {
  var params = [BridgedGenericTypeParamDecl]()
  for type in canGenericSig.genericSignature.genericParameters {
    assert(type.isGenericTypeParameter)
    params.append(BridgedGenericTypeParamDecl.createImplicit(declContext: declContext,
      name: type.bridged.GenericTypeParam_getName(),
      depth: type.bridged.GenericTypeParam_getDepth(),
      index: type.bridged.GenericTypeParam_getIndex(),
      paramKind: type.bridged.GenericTypeParam_getParamKind()))
  }
  return params.withBridgedArrayRef{BridgedGenericParamList.createParsed(astContext, leftAngleLoc: swift.SourceLoc(raw: nil), parameters: $0, genericWhereClause: BridgedNullableTrailingWhereClause(raw: nil), rightAngleLoc: swift.SourceLoc(raw: nil))}
}
...
func autodiffSpecializeBranchTracingEnum(
  ...
  let declContext = oldED.asDecl.getDeclContext()
  ...
  for enumCase in bteType.getEnumCases(in: topVJP)! {
    ...
    for idxInPayloadTuple in 0..<oldPayloadTupleType.tupleElements.count {
      ...
      if let closureInfoMultiBB = closureInfosMultiBB.first(where: { $0.idxInPayload == idxInPayloadTuple }) {
        ...
        if idxInPayloadTuple == 0 && oldPayloadTupleType.tupleElements[idxInPayloadTuple].isBranchTracingEnumIn(vjp: topVJP) {
    ...
    let newPL = [newPD].withBridgedArrayRef {
      BridgedParameterList.createParsed(astContext, leftParenLoc: swift.SourceLoc(raw: nil), parameters: $0, rightParenLoc: swift.SourceLoc(raw: nil))
    }
    newPLs.append(newPL)

    if newECDNameSuffix.count != 0 {
      newEDNameStr += "_\(oldEED.nameStr)\(newECDNameSuffix)"
    }
  }
  ...
  if !canGenericSig.isEmpty {
    genericParams = BridgedNullableGenericParamList(raw: cloneGenericParameters(astContext: astContext, declContext: declContext, canGenericSig: canGenericSig).raw)
  }

  let newED = newEDNameStr.withCString { strPtr in
    [AST.`Type`]().withBridgedArrayRef{inheritedTypes in BridgedEnumDecl.createParsed(
  ...
  for (idx, enumCase) in bteType.getEnumCases(in: topVJP)!.enumerated() {
    ...
    let newEED = BridgedEnumElementDecl.createParsed(
      ...
      name: oldEED.baseIdentifier, nameLoc: swift.SourceLoc(raw: nil), parameterList: BridgedNullableParameterList(raw: newPL.raw), equalsLoc: swift.SourceLoc(raw: nil), rawValue: BridgedNullableExpr(raw: nil))
...
func autodiffClosureSpecialization(function: Function, context: FunctionPassContext) {
  ...
  repeat {
    ...
    if isMultiBBWithoutBranchTracingEnumPullbackArg {
      log("Dumping VJP and PB at round \(remainingSpecializationRounds) begin")
      dumpVJPAndPB(vjp: function, pb: specializedFunction)
      log("Dumping VJP and PB at round \(remainingSpecializationRounds) end")
    }
...
private func rewriteUsesOfPayloadItem(
  ...
  case let uedi as UncheckedEnumDataInst:
    ...
    let newUedi = builder.createUncheckedEnumData(
      ...
      resultType: result.type.getEnumCasePayload(
        caseIdx: uedi.caseIndex, function: uedi.parentFunction
      ))
...
private func rewriteApplyInstructionCFG(
  ...
  for bb in vjp.blocks {
    ...
    let newArg = specializeBranchTracingEnumBBArgInVJP(arg: arg, specBTEDict: enumDict, context)
    arg.replaceAllUsesWith(newArg: newArg)
...
extension Cloner where Context == FunctionPassContext {
  fileprivate func cloneAndSpecializeFunctionBodyCFG(
    using pullbackClosureInfo: PullbackClosureInfo, enumDict: SpecBTEDict
  ) {
    let closureInfos = pullbackClosureInfo.closureInfosMultiBB
    ...
    var args = [Value]()
    for arg in self.getOrCreateEntryBlock().arguments {
      args.append(arg)
    }
    ...
    bbVisited[self.targetFunction.entryBlock] = true
    var bbQueue = [BasicBlock]()
    bbQueue.append(self.targetFunction.entryBlock)
    while bbVisited.count != self.targetFunction.blocks.count {
      for bb in self.targetFunction.blocks {
    ...
    for bb in bbQueue {
      ...
      if bb == self.targetFunction.entryBlock && self.targetFunction.blocks.singleElement == nil {
        ...
        self.context.erase(instruction: sei)

        continue
      }

      guard
        let (arg, enumTypeAndCase) = getBTEPayloadArgOfPbBBWithBTETypeAndCase(
      ...
      let newArg = specializePayloadTupleBBArgInPullback(
        arg: arg, enumTypeAndCase: enumTypeAndCase, context)
      arg.replaceAllUsesWith(newArg: newArg)
      bb.eraseArgument(at: arg.index, context)
        ...
        && newArg.uses.singleUse!.instruction as? DestructureTupleInst != nil
      {
        ...
        self.context.erase(instruction: oldDti)
        continue
      }
  ...
  private func cloneEntryBlockArgsWithoutOrigClosuresCFG(
    usingOrigCalleeAt pullbackClosureInfo: PullbackClosureInfo, enumDict: SpecBTEDict
  ) {
    let pb = pullbackClosureInfo.pullbackFn
...
private func getSpecializedParametersCFG(
  ...
  for paramInfo in applySiteCallee.convention.parameters {
    if paramInfo.type != enumType.mapTypeOutOfContext().canonicalType {
    ...
    let newParamInfo = ParameterInfo(
      type: enumDict[enumType]!.mapTypeOutOfContext().canonicalType,
...
func specializeBranchTracingEnumBBArgInVJP(arg: Argument, specBTEDict: SpecBTEDict, _ context: FunctionPassContext) -> Argument {
  return arg.parentBlock.insertPhiArgument(atPosition: arg.index, type: specBTEDict[arg.type]!, ownership: arg.ownership, context)
}
...
func specializePayloadTupleBBArgInPullback(
  ...
  let newPayloadTupleTy = newEnumType.getEnumCases(in: arg.parentFunction)!.filter{ $0.index == enumTypeAndCase.caseIdx }.singleElement!.payload!
  let oldOwnership = arg.ownership
